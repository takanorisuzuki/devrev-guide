...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    ...
    description: isJa
      ? '公式情報ベースでDevRevを15セッションで学ぶ。既存システムを保ちながらMemory・検索・統制を積み上げる視点と、Claude等LLM単体で使うよりも、DevRevと組み合わせることで、より速く正確で低コストな活用方法を扱う。'
      : 'Learn DevRev in 15 sessions from official sources. Compared to using LLMs (e.g. Claude) alone, combining them with DevRev delivers faster, more accurate, lower-cost results.',
    openGraph: {
      title: isJa ? 'DevRev Guide - 学習サイト' : 'DevRev Guide - Learning Site',
      description: isJa
        ? '公式情報をもとにDevRevを15セッションで学ぶ。Memory・権限・根拠の基盤に加え、Claude等LLM単体で使うよりも、DevRevと組み合わせることで、より速く正確で低コストな業務活用方法も扱う。'
