...
**なぜ Computer AirSync 取込が Internal デフォルトなのか**: 外部ツールから同期したドキュメントには社内限定の情報が含まれることが多い。Computer for Your Customers や Support Portal に意図せず公開されることを防ぐため、Internal scope（=private）がデフォルト適用される。
...
- Support Portal や Computer for Your Customers からの検索対象になる
- SEO インデックスの対象になる
...
一部の Computer AirSync Extractor は、元システムの権限設定を保持したまま同期する **Permission Aware** 機能を持つ。
...
- Computer AirSync 取込の記事は、デフォルトで Computer for Your Customers/Portal に露出しない設計になっているため、公開したい場合は明示的に `shared_with` を設定する必要がある
