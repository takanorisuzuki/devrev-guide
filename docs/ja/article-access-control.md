...
ナレッジベース（KB）の Article は「誰に見せるか」を精密に制御できます。[s05](/ja/s05) ではCollection公開の基本を扱いましたが、このページでは**権限モデルの全体像**をリファレンスとしてまとめます。

## 3つの制御パラメータ

Article のアクセス制御は、以下の3つのパラメータで決まります。

| パラメータ | 役割 | 設定方法 |
|-----------|------|---------|
| **scope** | 記事の大分類（社内向け / 顧客向け） | 作成経路で自動決定（手動作成=external, Computer AirSync取込=internal） |
...
この3つは独立ではなく、**内部的に同期されています**。API リクエストでは `access_level` と `shared_with` のどちらか一方のみ送信可能です（両方同時は不可）。
...
| 主な作成経路 | **Computer AirSync** による取込（Confluence, Notion, OneDrive等） | GUI で手動作成、またはURLクローリング |
| Computer for Your Customers への露出 | デフォルトで非公開（意図しない公開を防ぐ設計） | 設定に応じて公開される |

**なぜ Computer AirSync 取込が Internal デフォルトなのか**: 外部ツールから同期したドキュメントには社内限定の情報が含まれることが多いです。Computer for Your Customers や Support Portal に意図せず公開されることを防ぐため、Internal scope（=private）がデフォルト適用されます。
...
> 現時点で実運用上重要なのは **`private`** と **`public`** の2つです。

### private の動作

- デフォルトのシステムロール（seeded roles）が**一切適用されません**
- `shared_with` に明示的に追加されたユーザー/グループ**のみ**がアクセス可能です
- Internal scope の記事はすべて `access_level=private` がデフォルトです
...
- 「Visible to」を空にすると、Internal scope 記事は作成者のみアクセス可能になります

### グループによるアクセス制御の実例

RevUser 型のグループ（例:「All Customers」「Partner Group」）を `shared_with` に指定することで、顧客セグメント別にArticleの公開範囲を制御できます。
...
Article へのアクセスリクエストが来たとき、以下の順序で判定されます。
...
| Computer for Your Customers/Portal への露出 | デフォルトでは**非公開** | Published にすると**公開** |
| 変更方法 | `shared_with` を API/GUI で追加 | 「Visible to」で制限を追加 |

### Computer AirSync 取込で顧客に公開する手順

Computer AirSync で Internal scope として取り込まれた記事を顧客に公開したい場合:

1. Article の `shared_with` に対象の RevUser グループを追加します
2. または、Article を手動で再作成し External scope にします
...
- `shared_with` と `access_level` は内部的に同期されているため、**APIで片方を変更するともう片方も影響を受けます**
- `scope` は作成後に変更できません（Internal→External への変更は、記事の再作成が必要です）
- GUI 上では `access_level` の値を直接確認・変更する手段がありません（APIで確認します）
- Computer AirSync 取込の記事は、デフォルトで Computer for Your Customers/Portal に露出しない設計になっているため、公開したい場合は明示的に `shared_with` を設定する必要があります
