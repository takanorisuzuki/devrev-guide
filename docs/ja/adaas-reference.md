...
| プロパティ | 必須 | 説明 |
|-----------|------|------|
| `controlling_fields` | ✅ | ステージをエンコードする外部フィールド名 |
...
| モード | 説明 | 抽出ロジック |
|--------|------|------------|
| `SyncMode.INITIAL` | 初回同期。全データ抽出 | `extract_from` を開始タイムスタンプとして使用 |
...
- [ ] 2つの異なる顧客が同じ `external_system_id` にならない
- [ ] 同一顧客内の異なるユーザーが同じ `external_system_id` になる
...
| # | 失敗 | 影響 | 対策 |
|---|------|------|------|
| 1 | `external_org_id` をハードコード | 顧客間でデータ重複 | 外部APIから動的に取得する |
| 2 | 変更可能な値を `external_org_id` に使用 | リネーム時にデータ重複 | GUID等の不変IDを使用 |
