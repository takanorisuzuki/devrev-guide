...
## マーケットプレイス既製Snap-inカタログ（2026年6月時点）

自作する前に、マーケットプレイス（https://devrev.ai/marketplace）で既製品が存在しないか確認してください。以下は主要な既製Snap-inの一覧です。

### インテグレーション（外部システム接続）

| Snap-in | 機能 |
|---------|------|
| **Slack** | Slackチャンネルの内容をComputerにインポートし、AI検索で活用（ナレッジベース同期） |
| **Slack for Support** | Slack内からチケット作成・メッセージ双方向同期。顧客サポートをSlack上で完結 |
| **GitHub** | PR・コミットとIssueの自動連携 |
| **Bitbucket** | Bitbucketリポジトリとの連携 |
| **Harness** | CI/CDパイプラインとの連携 |
| **Datadog** | モニタリングアラートの連携 |
| **PagerDuty** | インシデントアラートをIssueとして取り込み |
| **Calendly** | ミーティング予約の連携 |
| **Email** | メールからのチケット作成と返信 |
| **WhatsApp** | WhatsAppメッセージとの連携 |
| **Instabug** | モバイルアプリのバグレポート連携 |
| **Google Mail** | Gmailとの連携 |
| **Google Calendar** | カレンダーイベントとの連携 |

### AirSync（データ同期）

| Snap-in | 機能 |
|---------|------|
| **Jira** | IssueとDevRev Issueの双方向同期。共存モード対応 |
| **Salesforce** | Account/Contact/Opportunityの同期 |
| **Zendesk** | チケットの同期。移行パスとして利用 |
| **Linear** | LinearのIssue同期 |
| **ClickUp** | タスクの同期 |
| **Azure Boards** | Azure DevOps Work Itemsの同期 |
| **ServiceNow / Jira Service Management** | ITSMチケットの同期 |
| **Confluence** | ナレッジベース記事の同期 |
| **Rocketlane** | プロジェクト管理データの同期 |
| **HubSpot** | コンタクト・会社情報の取り込み |
| **GitBook** | ドキュメントの同期 |
| **Paligo** | テクニカルドキュメントの同期 |
| **Qase** | テストケースの同期 |
| **DevRev** | DevRev間のデータ同期（クロスリージョン等） |
| **Airtable** | Airtableデータの取り込み |
| **Tracxn** | スタートアップ情報の同期 |

### オートメーション（DevRev内の自動化）

| Snap-in | 機能 |
|---------|------|
| **Convergence** | チケット→Issue変換の自動化ルール |
| **Auto-reply / Automatic Customer Reply** | 営業時間外の自動返信 |
| **Spam Shield** | スパムチケットの自動検出・フラグ |
| **Sentiment Evaluator** | 顧客メッセージの感情分析 |
| **StageFlow Automator** | ステージ遷移の自動化 |
| **Smart Issue Creator** | チケットから自動でIssueを作成 |
| **CSAT on Conversation / Ticket** | 顧客満足度アンケートの自動送信 |
| **SLA Change Notifier** | SLA違反が近づいた際のSlack通知 |
| **Slack Notifier Agent** | 設定に基づいたデータサマリーを毎日Slackに自動投稿 |
| **Ticket Approval Workflow** | チケットの承認ワークフロー |
| **Account Deduplication** | 重複アカウントの自動検出・マージ |
| **Slash Commands** | カスタムスラッシュコマンド基盤 |
| **Work Duration** | 作業時間の自動記録 |
| **Operational SLA Metrics** | SLAメトリクスの自動計測 |

> **Note**: マーケットプレイスは継続的に更新されています。最新の一覧は https://devrev.ai/marketplace を確認してください。
