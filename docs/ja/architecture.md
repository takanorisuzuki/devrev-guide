...
Memory のナレッジグラフは **事前定義されたオントロジー** を持つ。インテグレーション（Computer AirSync）がデータを取り込む時点で、プラットフォームは「顧客アカウント」「サポートチケット」「プロダクトパーツ」「エンジニアリング Issue」が何であり、それらがどう関係するかを既に知っている。
...
Computer AirSync（旧 Airdrop）は外部システムとの双方向同期エンジンである。50 以上のツール（Salesforce, Jira, Zendesk, Slack, GitHub, Google Workspace 等）からデータを Memory に取り込む。
