...
チュートリアルでは [s03](/ja/s03) で概念を学び、このページでは **一覧・図・リンクルール** をいつでも引ける形でまとめる。

## 全体リレーション図

主要なオブジェクト群の関係の目安である（詳細は製品の設定により異なる場合がある）。
...
graph TB
    ...
    subgraph RevParts["RevParts（顧客向け製品構成）"]
    ...
    subgraph DevParts["DevParts（内部技術構成）"]
    ...
    RevParts --> Article["Article（ナレッジベース）"]
    Identity --> RevParts
    Identity --> Work
    Identity --> DevParts
```
...
| RevParts | Product | 製品最上位 | ○ | ○（参照） |
| RevParts | Capability | 機能領域 | ○ | ○（参照） |
| RevParts | Feature | 個別機能 | ○ | ○（参照） |
| RevParts | Enhancement | 改善・大きなテーマ | ○ | × |
| DevParts | Code / Service | 内部サービス | ○ | × |
| DevParts | Runnable | 実行可能サービス | ○ | × |
| DevParts | Linkable | ライブラリ等 | ○ | × |
...
| Article | Part | （必須紐づけ） | KB は RevPart に必ず紐づける |
...
| Part ↔ Part | is_parent_of / is_child_of | RevPart 階層構造 |
...
| RevParts / DevParts | 顧客向け構成と内部技術を分離 | 顧客には製品の見え方だけを伝えやすい |
