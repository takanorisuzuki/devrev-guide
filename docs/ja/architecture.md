...
チュートリアルでは [s03](/ja/s03) で概念を学びます。公式の全体説明は [Core concepts](https://support.devrev.ai/devrev/article/ART-21847) にあります。このページは **一覧・図・リンクルール** をいつでも引けるリファレンスです。

## 全体リレーション図

3本柱は **Identity → Parts → Work** です。
...
**Enhancement** は **Part（customer part / RevPart）** として **Product → Capability → Feature → Enhancement** の階層に属します（**Issue / Ticket そのものではありません**）。

一方で Enhancement は、次のように **Work に近い振る舞い**も併せ持ちます（[s03](/ja/s03) 参照）。

- **is_parent_of** により、複数 Issue を束ねる（Epic 的なまとまり）
- Issue / Ticket は **is_work_of** で Part に帰属する（**Feature** だけでなく **Enhancement** が帰属先になることもある）

図ではこの2つを示しています。**Incident → Ticket** の向きは下記リンクルール表と一致させています。詳細は製品の設定により異なる場合があります。
...
主要オブジェクトをカテゴリ別に示します。DevUser / RevUser の可視性は目安です。
...
| その他 | Article | ナレッジ（[権限制御の詳細](/ja/reference/article-access-control)） | ○ | ○（公開分） |
| CRM | Opportunity | 商談 | ○ | × |
...
カスタムリンクタイプを独自に定義することも可能です。

## 拡張の仕組み（カスタムフィールド・サブタイプ・カスタムオブジェクト）

設計や API 連携の途中で「標準オブジェクトだけでは足りない」と感じたときに戻って読むセクションです。いますべてを設定する必要はありません。カスタムオブジェクトを作るべきかどうかの判断基準は [オントロジー設計ガイド](/ja/reference/ontology-design) を参照してください。

DevRev の標準オブジェクトでは表現しきれない業務要件がある場合、以下の 3 つの拡張手段を使います。

### カスタムフィールド

既存オブジェクト（Ticket, Issue, Account など）にフィールドを追加します。UI の Settings 画面から設定可能です。
...
- フィールド名には `tnt__` プレフィックスが付きます（テナント固有を意味する）。
- 型は text, int, double, bool, enum, timestamp, id（他オブジェクト参照）などが使用可能です。
- enum 型は選択肢をあらかじめ定義します。

### サブタイプ

既存のオブジェクトタイプ（Ticket, Issue など）に対して、用途別のサブタイプを定義します。
...
サブタイプごとに異なるカスタムフィールドを持たせることができます。たとえば Bug サブタイプにのみ「再現手順」フィールドを追加する、といった設計が可能です。

### カスタムオブジェクト

標準のオブジェクトカテゴリに収まらない概念をモデル化する場合に使います。
...
- カスタムオブジェクトは `custom_object.xxx` という名前空間で管理されます。
- 標準オブジェクトと同様にリンクで接続でき、検索やダッシュボードの対象にもなります。
- API からの操作も標準オブジェクトと同じパターンで行えます。
...
カスタムフィールドの設定方法は Settings > Object Customization から行います。カスタムオブジェクトの作成は API 経由で行います（[s12](/ja/s12) 参照）。

## Computer Memory アーキテクチャ

[s01](/ja/s01) と [s14](/ja/s14) で触れた **Computer Memory** の技術的な仕組みを整理します。Memory はオブジェクト構造（上記リファレンス）の上に構築されています。

### Memory とは何か

Memory は DevRev の **AI ネイティブなナレッジグラフ基盤**です。単なるデータベースや検索インデックスではなく、複数の専門ストアを組み合わせた統合データレイヤーです。

> 「重要なのは、どの AI モデルを使うかではなく、そのモデルが何を推論の対象としているかである。」

### 構成要素

Memory は以下の専門コンポーネントで構成されています。プラットフォームがクエリの種類に応じて適切なストアにルーティングします。
...
一般的な AI システム（RAG）は、ドキュメントを検索して LLM が回答を合成します。DevRev の Memory は異なるアプローチを取ります。
...
LLM がドキュメントから推測するのではなく、構造化データに対して決定論的なクエリを実行し、その結果を読み上げます——これが「検索する」と「記憶している」の本質的な違いです。

### オントロジー: 事前定義されたデータモデル

Memory のナレッジグラフは **事前定義されたオントロジー** を持ちます。インテグレーション（Computer AirSync）がデータを取り込む時点で、プラットフォームは「顧客アカウント」「サポートチケット」「プロダクトパーツ」「エンジニアリング Issue」が何であり、それらがどう関係するかを既に知っています。

これはこのページ上部で整理したオブジェクト構造そのものです。
...
これらの関係はクエリ時に推論されるのではなく、グラフ上のファーストクラスのエンティティとして永続的に保持されています。

### Computer AirSync: データが Memory に入る仕組み

Computer AirSync（旧 Airdrop）は外部システムとの双方向同期エンジンです。50 以上のツール（Salesforce, Jira, Zendesk, Slack, GitHub, Google Workspace 等）からデータを Memory に取り込みます。
...
Memory は特定の LLM に依存しない設計です。

- 構造化クエリ（SQL）の実行に LLM は不要です
- LLM はユーザーの自然言語を SQL に変換する部分と、結果を人間向けに整形する部分で使用されます
- LLM プロバイダーを切り替えても Memory 層は影響を受けません
- 顧客データは LLM のトレーニングに使用されません

### Context Registry: AI のための「地図の凡例」

Context Registry は Memory の中でも特に重要なコンポーネントです。

生のデータベーススキーマだけでは、AI は「チケット ID」と「ユーザー ID」の区別がつきません。Context Registry はスキーマにアノテーション（フィールドの説明、許容値、セマンティクスの文脈）を付与し、エージェントが自律的にデータを理解して判断できるようにします。

これが NL → SQL 変換の精度を支えている仕組みです。

### このセクションの位置づけ

| 学習場所 | 内容 |
|---------|------|
| [s01](/ja/s01) | Computer の 4 基盤の概要（Computer AirSync / Memory / Foundational Services / Agent Studio） |
...
オブジェクトがなぜこのように分かれているか、学習用の観点で整理しました。
...
| Article はアクセス制御 | 公開・内部・非公開などレベルで制御（[詳細](/ja/reference/article-access-control)） | 社内ドキュメントと顧客向けヘルプを同一基盤で管理できる |
